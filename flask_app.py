...
def dashboard():
    ...
    <head>
        ...
        <style>
            ...
            .chart-area { min-width: 1100px; height: 300px; }
    ...
    <body>
        <div class="container">
            ...
            <div class="stats-grid">
                <div class="stat-card"><div class="stat-label">Etat Global</div><div class="stat-value" style="color: {{ '#4ade80' if last_status == 1 else '#f87171' }}">{{ "OK" if last_status == 1 else "ERREUR" }}</div></div>
            ...
            <div class="chart-container-wrapper"><div class="chart-area"><canvas id="latencyChart"></canvas></div></div>
            <div class="card">
                <table>
                    <thead><tr><th style="padding-left:20px">ID</th><th>Date</th><th>Latence</th><th>Statut</th></tr></thead>
                    <tbody>
                        {% for run in runs %}
                        <tr>
                            <td style="padding-left:20px">#{{ run[0] }}</td><td>{{ run[1] }}</td><td>{{ run[3] }} ms</td>
                            <td class="{{ 'status-ok' if run[4] == 1 else 'status-err' }}">{{ "Operationnel" if run[4] == 1 else "Echec" }}</td>
                        </tr>
                        {% endfor %}
                    </tbody>
                </table>
